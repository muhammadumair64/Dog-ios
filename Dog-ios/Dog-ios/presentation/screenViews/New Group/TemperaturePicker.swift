...
//  Dog-ios
//
//  Created by Mac Mini on 27/06/2025.
//
    ...
    var body: some View {
        ZStack {
            ...
            HStack(spacing: 10) {
                ...
                ZStack {
                    // Curved line with integrated tick marks
                    SliderTrackView(
...
struct SliderTrackView: View {
    ...
    private func getCurvedPosition(at y: CGFloat, centerX: CGFloat, curveAmount: CGFloat) -> CGFloat {
        let curveStart = max(0, thumbPosition - 30)
        let curveEnd = min(sliderHeight, thumbPosition + 30)
        
        if y >= curveStart && y <= curveEnd {
            // Calculate how far we are in the curve section (0 to 1)
            let curveProgress = (y - curveStart) / (curveEnd - curveStart)
            
            // Use quadratic bezier curve formula to get x position
            let t = curveProgress
            let p0 = CGPoint(x: centerX, y: curveStart)
            let p1 = CGPoint(x: centerX + curveAmount, y: thumbPosition)
            let p2 = CGPoint(x: centerX, y: curveEnd)
    ...
    var body: some View {
        Canvas { context, size in
            ...
            // Draw tick marks first (behind the main line)
            for temp in Int(minTemp)...Int(maxTemp) {
                let progress = Double(temp - Int(minTemp)) / totalRange
                let y = sliderHeight - (CGFloat(progress) * sliderHeight)
                ...
                // Get the curved x position for this y coordinate
                let curvedX = getCurvedPosition(at: y, centerX: centerX, curveAmount: curveAmount)
                
                // Draw tick mark perpendicular to the curve
                var tickPath = Path()
                tickPath.move(to: CGPoint(x: curvedX + 3, y: y))
                tickPath.addLine(to: CGPoint(x: curvedX + 3 + tickWidth, y: y))
            ...
            // Draw the main curved track line
            var path = Path()
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                ...
                .overlay(
                    Image(ImageResource.thumb)
                        .resizable()
                        .scaledToFill()
                        .frame(width: 24, height: 24)
                        .clipShape(Circle()) // Ensures image stays within the circular boundary
                )
